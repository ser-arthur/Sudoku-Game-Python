...
class SudokuUI:
            ...
            for j in range(9):
                ...
                if grid[i][j] != 0:
                    if isinstance(grid[i][j], str):  # check if cell contains notes
                        ...
                        # Draw cell rect with background color
                        ...
                        text_rect = text.get_rect(center=(x0 + width // 2, y0 + height // 2))  # Center text within cell
                        self.screen.blit(text, text_rect)

                # Draw grid lines
                pygame.draw.rect(self.screen, grid_ln_color, (x0, y0, width, height), 1)

        # Draw thicker grid lines for the 9 '3x3 sub-grids'
        for i in range(9):
            for j in range(9):
                x0 = self.MARGIN + j * self.CELL_SIZE
                y0 = self.MARGIN + i * self.CELL_SIZE
    ...
    def draw_solve_button(self):
        ...
        pygame.draw.rect(self.screen, button_color, self.solve_button_rect,
                         border_radius=7)  # border_radius creates rounded corners
    ...
    def draw_lives(self):
        ...
        # Draw circles representing lives
    ...
    def draw_notes_button(self):
        ...
        pygame.draw.rect(self.screen, button_color, self.notes_button_rect)  # button
        pygame.draw.rect(self.screen, '#757575', self.notes_button_rect, width=3)  # border
    ...
    def draw_notes(self, notes_str, x0, y0, cell_width, cell_height):
        ...
        cell_margin = 5  # margin from cell edges
    ...
    def update_clock(self):
        ...
        # Update the clock only if the game is still ongoing
    ...
    def display_results(self):
        ...
        pygame.draw.rect(self.screen, '#FBECB2', text_rect)  # textbox
        pygame.draw.rect(self.screen, '#45474B', text_rect, 3)  # border

        # Blit the display text onto the screen with padding
    ...
    def solve_sudoku_board(self, grid):
        ...
        self.selected_cell = self.highlighted_cell = None  # removes highlight on selected cell

        # Find next empty cell
        empty_cell = self.game.find_empty_cell(grid)

        # End game when all cells are filled
        if not empty_cell:
        ...
        # Assign empty cell with the value from solution grid
        ...
        # Call method recursively until all cells are filled
        ...
        if self.notes_mode:
            if isinstance(self.grid[row][col], str):  # Check if cell already has notes i.e. is str type
                if str(num) not in self.grid[row][col]:
                    self.grid[row][col] += str(num)  # add new note
                else:
