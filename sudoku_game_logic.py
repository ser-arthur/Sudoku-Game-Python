...
class SudokuGame:
    ...
    def generate_complete_grid(self):
        ...
        def solve(grid, row, col):
            # Recursive function to fill the Sudoku grid cell by cell until a complete grid is generated

            if row == 9:  # Indicates end of grid
                return True

            next_row = row + 1 if col == 8 else row  # Move to the next row if we reach the end of the current row
            next_col = (col + 1) % 9  # Move to the next column
    ...
    def solve_sudoku(self, grid):
        ...
        # Use the grid passed as argument, or the instance grid
        ...
        if not empty_cell:
            if grid not in self.solutions:
                self.solutions.append(grid)  # Store puzzle solution
